...
def string_to_data(data_string):
    data = []
    lst = list(data_string)  # break string to char list
    for ele in lst:
        a = int(ele, 16)
        data.append(a)
    return data
...
def decode_rle(rle_data):
    flat_data = []
    i = 0
    for ele in rle_data[::2]:  # loop the odd numbers
...
def to_hex_string(data):  # data is a list of numbers <16
    s1 = ""
    for i in data:
        s1 += hex(i).split("x")[1]  # 0xF -> get string right of x or hex(i).split("x")[-1]
    return s1
...
def to_rle_string(rle_data):
    ...
    for i in range(len(list0)):
        s1 += str(list0[i]) + hex(list1[i]).split("x")[-1] + ":"
...
# Functions of menu options start here
def load_file():
    input_file = input("Enter name of file to load: ")  # input the file name
    file_data = console_gfx.load_file(input_file)
    return file_data
